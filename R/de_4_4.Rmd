...
valid_obs %>%
  summarise(
    ...
    cor_est = cor(conditionb, log(fold_change)),
    cor_bs = cor(bs_mean_conditionb, log(fold_change))
)
```

Filter based on things that are DE:
...
de_truth %>%
  select(target_id, nb_dispersion = dispersion) %>%
  inner_join(valid_obs, by = c("target_id")) %>%
  ggplot(aes(counts + nb_dispersion * counts ^ 2, dispersion * counts ^2)) +
    geom_point(alpha = 0.05) +
    scale_x_log10() +
    scale_y_log10()
```
...
First, let's look at how many transcripts have BS var >= GLM var

```{r,cache=TRUE}
valid_obs %>%
  group_by(is_de) %>%
  summarise(
    bs_var_gte_glm = mean(bs_se_conditionb >= se_conditionb, na.rm = TRUE),
    n_gte = sum(bs_se_conditionb >= se_conditionb, na.rm = TRUE),
    n_total = sum(!is.na(bs_se_conditionb >= se_conditionb)))
```
