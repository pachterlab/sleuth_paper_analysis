...
poisson_variance_filter$sleuth_poisson_variance <- mclapply(1:sim$n,
  function(i) {
    si <- sample_info[[i]]
    res <- run_sleuth(si, poisson_technical_variance = TRUE)
    res$so <- NULL
    res
  })

# tmp <- poisson_variance_filter
# poisson_variance_filter <- list()
# poisson_variance_filter$sleuth_poisson_variance <- tmp
...
each_filter_zero_variance <- c(each_filter, poisson_variance_filter)
